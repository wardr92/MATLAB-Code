function [QFmaxprime] = Natsounds_ASSR_Fmaps(ASSR_AllConditions, ModelW, freqbin);

% Only grab from predefined frequencies
ASSR_AllConditions = squeeze(ASSR_AllConditions(:,freqbin,:,:,:)); % Only grab ASSR from predefined sensors and frequencies

% Set up for topo map contrasts
repeatmat = permute(ASSR_AllConditions, [1 4 3 2]); % Re-arranges dimensions into channel x sub x condition x time bin array
weights = ModelW; % Grab current model weights
TimeBins = ['0to1';'1to2';'2to3';'3to4';'4to5';'5to6']; % Creates time bin names
for i = 1:size(repeatmat,4)
    [Fcontmat(:,i),rcontmat,MScont,MScs, dfcs] = contrast_rep(repeatmat(:,:,:,i),weights); % Runs contrasts across all sensors with full model
    SaveAvgFile(strcat('Fcontmat_',TimeBins(i,:)),'_Model_Conditions.atg'),Fcontmat(:,i); % Saves F contrasts
end 

% Compute permutation distribution for F critical value
for ii = 1:size(repeatmat,4)
    for iii = 1:8000 % Loops through a set number of permutations
        repeatmat_draw = repeatmat(:,:,:,ii); % Creates temporary variable for repeatmat
        for iv = 1:size(repeatmat,2) % Loops through each subject's repeatmat
            randomorder = randperm(4); % Creates randomized order for full 4 condition model
            repeatmat_draw(:, iv, :) = repeatmat(:, iv, randomorder); % Runs permutation on all 4 conditions in full model
        end 
        ...
        [Fcontmat(:,ii),rcontmat,MScont,MScs, dfcs] = contrast_rep(repeatmat_draw,weights); % F contrast for full 6 conditions
        temp = sort(Fcontmat(:,ii), 'descend'); % Sorts F values by largest to lowest
        Fmaxprime(iii,ii) = temp(3); % Saves 3rd highest F value
    end 
end
...
for v = 1:size(repeatmat,4)
    subplot(3,2,v)
    hist(Fmaxprime(:,v))
    QFmaxprime(v) = quantile(Fmaxprime(:,v), 0.95);
    xlabel('F value'), ylabel('Count'); % Applies labels to x and y axes 
    xline(QFmaxprime(v),'-k',{(QFmaxprime(v))}, 'linewidth', 2, 'labelverticalalignment', 'top', 'labelorientation', 'horizontal');
    title(strcat(TimeBins(v,:), '_seconds'), 'Interpreter','none') 
end 
sgtitle('Full Condition Model', 'FontSize', 30);
