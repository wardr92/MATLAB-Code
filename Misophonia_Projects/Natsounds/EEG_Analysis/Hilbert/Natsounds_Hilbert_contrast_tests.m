function [sensors] = Natsounds_Hilbert_contrast_tests(Hilbert_AllConditions, sensors);
...
[~, ~, ~, stats_miso_v_neut] = ttest(Hilbert_AllConditions(:, 1, :), Hilbert_AllConditions(:, 3, :), 'Dim', 3); % Miso vs Neutral
[~, ~, ~, stats_unpl_v_neut] = ttest(Hilbert_AllConditions(:, 2, :), Hilbert_AllConditions(:, 3, :), 'Dim', 3); % Unpl vs Neutral
[~, ~, ~, stats_plea_v_neut] = ttest(Hilbert_AllConditions(:, 4, :), Hilbert_AllConditions(:, 3, :), 'Dim', 3); % Plea vs Neutral

% Plot t-values
fig1 = figure('Renderer', 'painters', 'Position', [10 10 1920 1080]); % Sets up figure size 
taxis = -600:2:6000; % Creates a time axis
hold on
plot(taxis, stats_miso_v_neut.tstat); % Plot t-values from Miso vs Neutral
plot(taxis, stats_unpl_v_neut.tstat); % Plot t-values from Unpl vs Neutral
plot(taxis, stats_plea_v_neut.tstat); % Plot t-values from Plea vs Neutral
hold off
lgd = legend;
legend('Misophonia-Related vs Neutral','Unpleasant vs Neutral', 'Pleasant vs. Neutral')
...
xlim([-600,6000])
title ('Hilbert Data - t-test Contrasts');
...
[Fcontmat,rcontmat,MScont,MScs, dfcs]=contrast_rep(repmat,[2 1 -1 -2]); % Tests F contrast model (highest for Miso, next for unpl, then plea, and neut)
...
repmat2 = repmat(:,:,2:4);
[Fcontmat,rcontmat,MScont,MScs, dfcs]=contrast_rep(repmat2,[1 0 -1]); % Tests F contrast model (highest for unpl, then plea, and neut)
...
xlabel('Time (ms)'), ylabel('F-value'); % Apply labels to x and y axes 
xlim([-600,6000])
title ('Hilbert Data - F-test Contrast - All but Misophonia');
end 
