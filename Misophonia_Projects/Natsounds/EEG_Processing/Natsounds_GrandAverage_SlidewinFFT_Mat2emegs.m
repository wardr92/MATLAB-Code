function [] = Natsounds_GrandAverage_SlidewinFFT_Mat2emegs()
...
cd('/Volumes/CSEA HDD/Misophonia Projects/EEG/NatSounds/Analyses/EEG/ASSR/Grand Averages/GA_Slidewin_FFT') % Moves to correct folder
AmpTimes = getfilesindir(pwd, '*Slidewin_FFT_Amplitude_AveragedTrials*AllConditions.mat'); % Grabs all the Amp time bins
for ii = 1:size(AmpTimes,1) % Loops through all time bins
    load(getfilesindir(pwd, AmpTimes(ii,:))) % Loads current ASSR time Amp
    for iii = 1:size(Conditions) % Loops through all conditions in this time bin
        ConditionAmp_AveragedTrials = Amps(:,:,iii); % Saves the current condition's Amps for FFTs on all averaged trials
        AvgTrialsName = char(strcat('GrandAverage_Slidewin_FFT_Amplitude_AveragedTrials_', AmpTimes(ii,52:54), '_seconds_', Conditions(iii), '.at', (cellfun(@(v) v(end), Conditions(iii)))));
        SaveAvgFile(AvgTrialsName, ConditionAmp_AveragedTrials,[],[], 194.1748,[],[],[],[],1); % Saves AllTrials FFT as a .at file
    end
    AllConAmp_AveragedTrials = mean(Amps,3); % Averaged across all conditions
    AveragedTrialsName = char(strcat('GrandAverage_Slidewin_FFT_Amplitude_AveragedTrials_', AmpTimes(ii,52:54), '_seconds_AllConditions.at')); % Assigns new variable name
    SaveAvgFile(AveragedTrialsName, AllConAmp_AveragedTrials,[],[], 194.1748,[],[],[],[],1); % Saves AveragedTrials FFT as a .at file
end 
fprintf(['\n Script Finished \n'])
